...
public sealed class ItunesMetadataService : IApiMusicMetadataService
{
  ...
  public async Task<SongMetadataResult> SearchAsync(string query, CancellationToken ct)
  {
    try
    {
      var url = "https://itunes.apple.com/search?media=music&entity=song&limit=1&term=" + Uri.EscapeDataString(query);
      var resp = await _http.GetFromJsonAsync<ItunesSearchResponse>(url, ct);
      var first = resp?.Results?.FirstOrDefault();
      if (first == null) return new SongMetadataResult { Success = false, ErrorMessage = "No results" };
    ...
    catch (OperationCanceledException) { return new SongMetadataResult { Success = false, ErrorMessage = "Canceled" }; }
    catch (Exception ex) { return new SongMetadataResult { Success = false, ErrorMessage = ex.Message }; }
  }
}
